...
def view():
	if (request.vars["owner"]):
		...
		grid = SQLFORM.grid(
	        ...
	        create=False,
	        links=[
	        		dict(header='Join it!', body=lambda row: A('Give a speech', _href=URL("lecture", "new", vars={"conference":row.id})) ),
	        		dict(header='Show agenda!', body=lambda row: A('Take a look', _href=URL("agenda", vars={"conference":row.id})) ),
	        	],
	        csv=False,
	        maxtextlength=200,
	    )
